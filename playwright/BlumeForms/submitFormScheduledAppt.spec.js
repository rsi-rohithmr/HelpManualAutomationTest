...
const { TIMEOUT_IN_MSEC1, TIMEOUT_IN_MSEC3, TIMEOUT_IN_MSEC4 } = require('../POM/timeouts');
...
test.describe.serial('Blume Submit Form - Scheduled Appointment', () => {
	test.beforeAll(async () => {
		...
		// Create forms and healthcare service
		clinicalForm = await studyHelper.postClinicalForm();
		patientForm = await studyHelper.postRegistrationForm();
	...
	test.afterAll(async () => {
		...
		if (patientForm?.id) {
			await api.deleteResource('form', patientForm?.id);
		}
		if (clinicalForm?.id) {
	...
	test('Submit Patient Registration Form', async () => {
		await page.waitForTimeout(TIMEOUT_IN_MSEC3);
		await page.evaluate(() => window.localStorage.clear());
		await poManager.loginPage.loginBlume();
		await page.waitForTimeout(TIMEOUT_IN_MSEC3);
		// Find and click the access forms button for the specific patient
		const appointmentCard = page.locator('.css-12gu1fm').filter({ hasText: data?.patientName });
		await expect(appointmentCard).toBeVisible({ timeout: TIMEOUT_IN_MSEC1 });

		// Find the access forms button within the specific appointment card
		const accessFormsBtn = appointmentCard.locator('[data-testid="appointment-card-forms-btn"]');
		await expect(accessFormsBtn).toBeVisible({ timeout: TIMEOUT_IN_MSEC1 });
		await expect(accessFormsBtn).toBeEnabled();
		await accessFormsBtn.click();
		await page.locator('span').filter({ hasText: patientForm?.name }).click();
		await page.getByText('Option 1', { timeout: TIMEOUT_IN_MSEC1 }).click();
	...
	test('Submit Clinical Form', async () => {
		await page.locator('span').filter({ hasText: clinicalForm?.name }).click();
		await page.getByText('Option A', { timeout: TIMEOUT_IN_MSEC1 }).click();
		await page.route('**/blume-api/form/v2/*', route => route.continue());
		await page.getByTestId('next', { timeout: TIMEOUT_IN_MSEC1 }).click();
		await poManager.apiWaitUtils.waitForAPI('blume-api/form/v2/', 'PUT', TIMEOUT_IN_MSEC4);
	});
