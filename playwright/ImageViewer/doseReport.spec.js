...
test.describe.serial('Verify Dose Report', () => {
    test.beforeAll(async ({}) => {
        ...
        const filePath = path.relative(
            process.cwd(),
            path.join(__dirname, '../TestData/dicomImport/testDoseReport/1.dcm')
        );
        await Promise.all([
    ...
    test('Verify Dose Report', async ({}, testInfo) => {
		...
		// const screenshotPath = path.join(testInfo.outputDir, 'doseReport.png');
		// await viewportLocator.screenshot({ path: screenshotPath });

        const viewport = await poManager.imageViewer.getCornerstoneViewportByIndex(0);
        expect(viewport.properties).not.toBeNull();
        expect(viewport).toBeDefined();
        const doseReport = path.join(
            __dirname,
            '../../screenshots-testdata/doseReport.png'
        );
        const viewportLocator = page.locator('[data-testid="ImageViewerViewport"]').nth(0).locator('.cornerstone-canvas');
        ...
        'Compare the screenshot of the viewport image with the baseline image at "../../screenshots-testdata/doseReport.png". Ignore insignificant differences such as anti-aliasing, minor color shifts, and font rendering. Focus only on clearly visible changes in layout, text, overlays, or structure.',
        'Return ONLY a JSON object in the following format:\n\n{\n  "isScreenshotsMatching": boolean,\n  "differences": [string]\n}\n\nDo not include any explanation or commentary.'
        );
        console.log('Dose Report comparison result:', comparisonResult);
